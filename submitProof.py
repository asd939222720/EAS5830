...
def generate_primes(num_primes):
    ...
    if len(primes_list) != num_primes:
        raise ValueError(f"Only found {len(primes_list)} primes, expected {num_primes}")
...
def convert_leaves(primes_list):
    ...
    for p in primes_list:
        leaves.append(p.to_bytes(32, byteorder='big'))
    return leaves
...
def build_merkle(leaves):
    ...
    if not leaves:
        raise ValueError("Leaves list is empty")
...
def prove_merkle(merkle_tree, random_indx):
    ...
    """
    merkle_proof = []
    if random_indx < 0 or random_indx >= len(merkle_tree[0]):
        raise IndexError("random_indx out of range of leaves")
