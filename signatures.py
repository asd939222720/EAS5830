...
def _to_defunct_message(m):
    ...
    if isinstance(m, bytes):
        return encode_defunct(primitive=m)
    else:
...
def verify(m, public_key, signed_message):
    ...
    valid_signature = (w3.to_checksum_address(signer) ==
                           w3.to_checksum_address(public_key))  # True if message verifies, False if message does not verify
